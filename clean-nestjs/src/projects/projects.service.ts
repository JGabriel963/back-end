  ...
  async update(id: string, updateProjectDto: UpdateProjectDto) {
    ...
    if (updateProjectDto.started_at) {
      if (project.status === ProjectStatus.Active) {
        throw new BadRequestException('Cannot start active project');
      }

      if (project.status === ProjectStatus.Completed) {
        throw new BadRequestException('Cannot start completed project');
      }

      if (project.status === ProjectStatus.Cancelled) {
        throw new BadRequestException('Cannot start cancelled project');
      }
    ...
    if (updateProjectDto.cancelled_at) {
      ...
      if (project.status === ProjectStatus.Cancelled) {
        throw new BadRequestException('Cannot cancel cancelled project');
      }

      if (updateProjectDto.cancelled_at < project.started_at) {
        throw new BadRequestException(
          'Cannot cancel project before it started',
        );
      }
    ...
    if (updateProjectDto.finished_at) {
      if (project.status === ProjectStatus.Completed) {
        throw new BadRequestException('Cannot finish completed project');
      }

      if (project.status === ProjectStatus.Cancelled) {
        throw new BadRequestException('Cannot finish cancelled project');
      }
